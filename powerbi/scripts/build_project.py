...
measures = [
    ...
    measure('Mean Completion',f'AVERAGEX ( {completion}, institutions[grad_rate_bach_6yr] )','0.0"%"','Unweighted mean of institution rates, stored on a 0–100 scale. Not a student-level graduation rate.'),
    ...
    measure('Median Net Price','MEDIAN ( institutions[net_price] )','$#,0','2022–23; median of institution-level average net prices; in-state at public institutions.'),
...
inst['columns'] += [
    ...
    {'name':'Completion Bin','dataType':'string','type':'calculated','expression':'VAR r = institutions[grad_rate_bach_6yr] VAR lo = institutions[Completion Bin Sort] RETURN IF ( ISBLANK ( r ), BLANK (), FORMAT ( lo, "0" ) & IF ( lo = 95, "–100", "–<" & FORMAT ( lo + 5, "0" ) ) )','sortByColumn':'Completion Bin Sort','summarizeBy':'none'},
...
write('IR Lab.Report/definition/report.json',{'$schema':schema('report','2.0.0'),'themeCollection':{'customTheme':{'name':'IRLab.json','reportVersionAtImport':'2.0.0','type':'RegisteredResources'}},'resourcePackages':[{'name':'RegisteredResources','type':'RegisteredResources','items':[{'name':'IRLab.json','path':'IRLab.json','type':'CustomTheme'}]}],'settings':{'useStylableVisualContainerHeader':True,'exportDataMode':'AllowSummarized','defaultDrillFilterOtherVisuals':True}})
...
def visual(p,name,typ,title,pos,roles=None,subtitle=None,objects=None,sort=None):
    ...
    title_props=v['visual']['visualContainerObjects']['title'][0]['properties']
    title_props['fontColor']={'solid':{'color':lit('#14213d')}}
    title_props['fontSize']={'expr':{'Literal':{'Value':'20D' if name=='heading' else '12D'}}}
    styling=v['visual'].setdefault('objects',{})
    if typ=='card':
        styling['categoryLabels']=[{'properties':{'show':lit(False)}}]
        styling['labels']=[{'properties':{'labelDisplayUnits':{'expr':{'Literal':{'Value':'0D'}}},'fontSize':{'expr':{'Literal':{'Value':'28D'}}}}}]
    if typ=='slicer':
        styling['header']=[{'properties':{'show':lit(False)}}]
        if name=='cohort': styling['data']=[{'properties':{'mode':lit('Between')}}]
...
p=page('ReportSectionOverview','01 · Institution overview')
banner(p,'IR LAB / Institution overview','IPEDS 2023–24 collection • Cross-sectional institution data • Aid and awards: 2022–23; retention: fall 2022→2023')
...
visual(p,'lookup','tableEx','Institution lookup',(24,620,1232,258),{'Values':[cp('name','Institution'),cp('state','State'),cp('sector','Sector'),cp('headcount','12-month enrollment'),cp('bach_cohort','Bachelor cohort'),cp('grad_rate_bach_6yr','6-year completion'),cp('net_price','Net price (2022–23)')]})

p=page('ReportSectionDistributions','02 · Completion distributions','Distribution Eligible')
banner(p,'EXPLORE / The shape of a rate','Four-year institutions with reported six-year bachelor completion • Each institution has equal weight • Rates use a 0–100 scale')
...
p=page('ReportSectionRegression','03 · Pell and completion','Regression Eligible')
banner(p,'TEST / A line through the cloud','Four-year institutions • Bachelor cohort ≥30 • Complete Pell and completion values • Association across institutions; no causal claim')
cards(p,['Regression Institutions','Pell Slope','Pell R Squared'])
slicers(p)
visual(p,'scatter','scatterChart','Pell share and six-year completion',(24,238,826,400),{'Category':[cp('unitid','Institution ID')],'X':[ap('pct_pell','Pell share (%)')],'Y':[ap('grad_rate_bach_6yr','Six-year completion (%)')],'Tooltips':[cp('name','Institution'),ap('bach_cohort','Bachelor cohort')]},'One dot per institution • Slicer selections refit the OLS cards',objects={'categoryAxis':[{'properties':{'start':lit(0),'end':lit(100)}}],'valueAxis':[{'properties':{'start':lit(0),'end':lit(100)}}]})
...
p=page('ReportSectionPeers','04 · Peer groups')
banner(p,'MODEL / Peer groups','Fixed R snapshot • Four-year public/nonprofit • Bachelor cohort ≥100 • Five complete standardized features • k=4, seed=2023, 25 starts')
